...
export function CourseContent({ course, userId }: CourseContentProps) {
  ...
  // Show skeleton until auth is loaded to prevent hydration mismatches
  if (!isAuthLoaded) {
    return <Skeleton className="w-full h-full" />;
  }

  // Check if user has access to this course (only after auth is loaded)
  const hasAccess = hasTierAccess(userTier, course.tier);
  
  // Debug logging (remove in production) - only log after mount to avoid hydration issues
