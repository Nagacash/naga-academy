...
export function TutorWidget() {
  ...
  // Check if user has Ultra tier through:
  // 1. Clerk subscription plan
  // 2. User metadata (for admin upgrades)
  // 3. User tier hook (which handles both)
  const hasUltraPlan = has?.({ plan: "ultra" });
  const metadataTier = user?.publicMetadata?.tier as string | undefined;
  const isUltra = hasUltraPlan || metadataTier === "ultra" || userTier === "ultra";
